# Zeyuan Guo (郭泽远)

Ph.D. student at **[Beijing University of Posts and Telecommunications (BUPT)](https://english.bupt.edu.cn/)**, formally advised by **[Prof. Chuan Shi](https://shichuan.org/)** and currently conducting research under the guidance of **[Enmao Diao](https://www.diaoenmao.com/zh-cn/)**, affiliated with **GAMMALab**.

## About Me

I am a Ph.D. student at BUPT, where I previously received my bachelor's degree. My research broadly lies in machine learning, with current interests in graph machine learning, transformers, generative modeling, and graph foundation models.

Besides research, I also enjoy building small open-source tools that make everyday work or life more convenient :)

## Research Interests

- Graph machine learning
- Transformers and generative models
- Representation learning on structured data
- Graph foundation models


## Selected Publications

- **Graph Tokenization for Bridging Graphs and Transformers (ICLR 2026)**  
  Paper: https://openreview.net/forum?id=jCctxI1BGF  
  Project: https://github.com/BUPT-GAMMA/Graph-Tokenization-for-Bridging-Graphs-and-Transformers

- **Benchmarking Graph Foundation Models (KDD 2025)**  
  Paper: http://shichuan.org/doc/199.pdf  
  Project: https://github.com/BUPT-GAMMA/ggfm

## Open-source Tools

These projects are secondary to my academic work, but they reflect my interest in practical and useful tools.

- **Video Auto Translator (VAT)**  
  A tool for automating video translation workflows.  
  https://github.com/112292454/Video-Auto-Translator

- **VideoZip**  
  A lightweight utility project for video processing workflows.  
  https://github.com/112292454/VideoZip

- **textQuery**  
  A small utility project for text retrieval / query-related experimentation.  
  https://github.com/112292454/textQuery

## Links

- **Email**: guozeyuan@bupt.edu.cn
- **Google Scholar**: https://scholar.google.com/citations?user=dVrSlvAAAAAJ
- **GitHub**: https://github.com/112292454

<div align="center">
<picture>
  <source media="(prefers-color-scheme: dark)" srcset="https://raw.githubusercontent.com/112292454/112292454/output/github-contribution-grid-snake-dark.svg" />
  <source media="(prefers-color-scheme: light)" srcset="https://raw.githubusercontent.com/112292454/112292454/output/github-contribution-grid-snake.svg" />
  <img alt="github-snake" src="https://raw.githubusercontent.com/112292454/112292454/output/github-contribution-grid-snake.svg" />
</picture>
</div>

<div align="center">

![Python Badge](https://img.shields.io/badge/python-512BD4)
![Java Badge](https://img.shields.io/badge/java-512BD4)
![Static Badge](https://img.shields.io/badge/GO-FAFAFA?logo=go)
![C Badge](https://img.shields.io/badge/C-555555?logo=c&logoColor=fff&style=flat)

</div>

<div align="center">
<img align="" height="137px" src="https://github-readme-stats-git-masterrstaa-rickstaa.vercel.app/api?username=112292454&hide_title=true&hide_border=true&show_icons=true&include_all_commits=true&line_height=21text_color=000&icon_color=000&bg_color=0,ea6161,ffc64d,fffc4d,52fa5a&theme=graywhite" />
<img align="" height="137px" src="https://github-readme-stats-git-masterrstaa-rickstaa.vercel.app/api/top-langs/?username=112292454&hide_title=true&hide_border=true&layout=compact&langs_count=6&text_color=000&icon_color=fff&bg_color=0,52fa5a,4dfcff,c64dff&theme=graywhite" /><br><br>

</div>
